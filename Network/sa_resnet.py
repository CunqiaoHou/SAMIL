...
class sa_layer(nn.Module):
    ...
    def forward(self, x):
        ...
        # spatial attention
        xs = self.gn(x_1)  #[2048, 2, 56, 56]
        xs = self.sweight * xs + self.sbias  #[2048, 2, 56, 56]
        xs = x_1 * self.sigmoid(xs)  #[2048, 2, 56, 56]
...
class SABottleneck(nn.Module):
    ...
    def forward(self, x):
        identity = x
        # print('SABottleneck:', identity.shape)
        out = self.conv1(x)
        out = self.bn1(out)
        out = self.relu(out)
